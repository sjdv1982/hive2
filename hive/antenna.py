...
class PullAntenna(AntennaBase, TriggerTarget):
    ...
    def _hive_cornnectable_target(self, source):
        asset isinstance(source, Output) # TODO : nicer error message
...
class AntennaBee(HiveBee, Antenna, ConnectTarget, TriggerSource, Exportable):
    ...
    def getinstance(self, hive_object):
        ...
        if self.mode == "push":
            return PushAntenna(target, self.data_type)

        else:
            return PullAntenna(target, self.data_type)
    ...
    def implements(self, cls):
        ...
        if cls is TriggerTarget and self.mode == "pull":
            return True
