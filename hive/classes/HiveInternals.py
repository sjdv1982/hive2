...
class HiveInternals(object):
  ...
  def __setattr__(self, name, value):
    ...
    if isinstance(value, Exportable) and not isinstance(value, HiveObject):
      raise TypeError("HiveInternals (i) attribute '%s' must not be Exportable; Exportables must be added to ex" % name)
    if value._hivecls is None:
      raise AttributeError("HiveInternals (i) attribute '%s' must contain a Bee built by '%s' (or one of its base classes), but the Bee was not built by any hive" \
        % (name, self._hivecls.__name__))      
    if not issubclass(value._hivecls, self._hivecls):
      raise AttributeError("HiveInternals (i) attribute '%s' must contain a Bee built by '%s' (or one of its base classes), not '%s'" \
